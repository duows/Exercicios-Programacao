...
int main() { _
	...
	int n; cin >> n;
    string carac; cin >> carac;
    string word; cin >> word;
    double tempo; cin >> tempo;

    map<char, int> conversor;
    ...
    AAACB
    A*6^4 + A*6^3 + A*6^2 + C*6^1 + B*6^0
    0*6^4 + 0*6^3 + 0*6^2 + 2*6^1 + 1*6^0 = 0 + 0 + 0 + 12 + 1 = 13
    13 * tempo = 13 * 2 = 26.00

    */

    int expo = word.size() - 1;
    int res = 0;
    for(auto c : word) {
        if(conversor.find(c) != conversor.end()) {
            int valor = conversor[c];
            double resultado = valor * pow(n, expo);
            res += resultado;
        }
        expo--;
    }

    cout << fixed << setprecision(2) << tempo * res << endl;
