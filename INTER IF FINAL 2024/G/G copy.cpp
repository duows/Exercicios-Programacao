...
int main() { _
    ...
    // Vetor para conversão mais rápida (ASCII index)
    vector<int> conversor(128, -1);
    for (int i = 0; i < n; ++i) {
        conversor[carac[i]] = i;
        // cout << carac[i] << " -> " << conversor[carac[i]] << endl; // Debugging
    }

    // Convertendo palavra para valor na base-n sem usar pow
    long long res = 0;
    for (char c : word) {
        res = res * n + conversor[c];
    }
